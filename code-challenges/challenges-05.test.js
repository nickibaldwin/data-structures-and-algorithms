...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  return starWarsArr.sort(compare);
};
...
const removeThree = (idx, arr) => {//declare the function named removeThree, which is taking in inx and arr.
  // Solution code here...
  arr.splice(idx, 3);//.splice(start: at this index ie 2 as seen above, deleteCount:we are removing 3, therefore it's 3)
  return arr;
};
...
const joinArray = (arr) => {//declare joinArray
  // Solution code here...
...
const listFoods = (recipe) => {
  ...
  for (var i = 0; i < recipe.ingredients.length; i++) {
    var a = recipe.ingredients[i].indexOf(' ');
    var b = recipe.ingredients[i].indexOf(' ', a + 1);
    result.push(recipe.ingredients[i].slice(b + 1));
  }
  return result;
};
