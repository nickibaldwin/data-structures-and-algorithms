...
const containsAnd = (arr) => {
    ...
    //     if (value.match(/and/g)) {
    //         return value;
    //     }
    // })
    // return results;
    return arr.filter(value => value.match(/and/g));
};
...
const filterStringsWithVowels = (arr) => {
    // Solution code here...
    return arr.filter(value => value.match(/[aeiou]/g));
};
...
const filtered = arr.filter(function(notIncluded))
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
    ...
    const output = arr.filter((value) => {;
        //need to return truethy or falsey
        if (value.baseState > minBaseStat) return true;
        return false;
    });
    return output;
};
...
const getStatName = (arr, minBaseStat) => {
    ...
    const outputPart1 = arr.filter(filteringCallback); //filter first
    ...
    const outputPart2 = outputPart1.map(thing => thing.stat.name); //map it into just the strings
    return outputPart2;
};
