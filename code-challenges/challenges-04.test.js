...
const addTwo = (arr) => { //delcaring a variable which is a function called addTwo which takes in an array as a parameter

    // Solution code here...
    const newArr = [];

    arr.forEach(number => { newArr.push(number += 2) }); //forEach. need at least 1 parameter (no parens), multi would have parense
    return (newArr);
};
...
const isNum = (input) => {
    // Solution code here...
    const regTest = /\d/g;
    const outputTest = regTest.test(input);
    if (outputTest) {
        return true;
    } else {
        return false;
...
const citiesAtoJ = (arr) => {
    ...
    const arrNew = [];
    arr.forEach(value => {
        const str = value.match(/^[A-J]\w+/gm);
        if (str) {
            arrNew.push(str[0]);
        }
    });
